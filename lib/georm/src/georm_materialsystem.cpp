                    ...
                    if (type == "uniform") {
                        if (m_uniformBlockManagers.count(name) <= 0) {
                            throw std::runtime_error(fmt::format(
                                "Material {}: Uniform block '{}' is not registered to the material system. Shader will likely misbehave.", m.name, name));
                        } else {
...
std::shared_ptr<geo::Material> MaterialSystem::createMaterial(const std::string& name) {
    ...
    if (m_materials.count(materialName) > 0) {
        ...
        if (m_materials.count(m_defaultMaterial) > 0) {
            ...
            spdlog::warn("Material '{}' is not defined. No default material is set. Using the first defined material (0: '{}') instead.", name,
                         material.name);
        } else {

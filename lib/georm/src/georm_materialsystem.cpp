...
void MaterialSystem::loadMaterialsFromFile(const std::filesystem::path& path) {
  ...
  if (input.fail()) {
    throw std::runtime_error(fmt::format("MaterialSystem fails to initialize: cannot read from '{}'", path.string()));
  }
  ...
  if (j.contains("default_material")) {
    ...
    if (m_materials.count(defaultMaterial) > 0) {
      ...
      spdlog::warn("{} specifies a default material '{}' that is not defined in the list of materials", path.string(), defaultMaterial.get<std::string>());
    }
  }
}

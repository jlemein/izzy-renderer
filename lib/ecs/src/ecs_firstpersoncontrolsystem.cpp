...
void print(const char* str, const glm::mat4& m) {
  std::cout << str << std::endl;
  for (int i = 0; i < 16; ++i) {
    int c = i % 4;
    int r = i / 4;
    std::cout << m[r][c] << " ";
    if (c == 3) std::cout << std::endl;
  }
}

void print(const char* str, const glm::vec4& v) {
  std::cout << str << std::endl;
  for (int i = 0; i < 4; ++i) {
    std::cout << v[i] << " ";
  }
...
ecs::FirstPersonMovementSystem::FirstPersonMovementSystem(entt::registry& registry, io::InputSystem* inputSystem)
  : m_registry(registry)
  , m_inputSystem(inputSystem) {}

void ecs::FirstPersonMovementSystem::init() {
...
void ecs::FirstPersonMovementSystem::update(float dt) {
  ...
  for (auto e : view) {
    ...
    if (!fpActor.onlyRotateOnMousePress || m_inputSystem->isMouseButtonPressed(io::MouseButton::kLEFT)) {
    ...
    // adjust position
    auto velocity = m_registry.all_of<Moveable>(e) ? m_registry.get<Moveable>(e).velocity : fpActor.defaultVelocity;
    if (m_inputSystem->isKeyPressed(io::Key::kLEFT_SHIFT)) {
      velocity *= 3.0F;
    }
    ...
    glm::vec3 relativeMovement = cameraOrientation * glm::vec4(right * velocity * dt, 0.0F, -forward * velocity * dt, 1.0F);
    glm::mat4 translate = glm::translate(glm::mat4(1.0F), relativeMovement);

    cameraPosition += glm::vec3(translate[3]);

    transform.localTransform = glm::translate(glm::mat4(1.0f), glm::vec3(cameraPosition)) * cameraOrientation;
  }
}